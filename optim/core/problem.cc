...
// init the global index for the vertices
...
Problem::~Problem() {
    ...
    Vertex::global_vertex_id_ = 0; 
    Edge::global_edge_id_ = 0;
}
...
inline bool Problem::IsPoseVertex(std::shared_ptr<Vertex> v) {
    string type = v->TypeInfo();
    return type == string("VertexPose") ||
            type == string("VertexSpeedBias");
}

inline bool Problem::IsLandmarkVertex(std::shared_ptr<Vertex> v) {
    string type = v->TypeInfo();
...
//#endif
    ...
    // [FOLLOWING ARE MOVED TO SPEC PROBLEM]
    if(H_prior_.rows() > 0) 
    {
...
void Problem::SolveLinearSystem() {
    ...
    if (problemType_ == ProblemType::GENERIC_PROBLEM) {
        ...
        //[FOLLOWING THINGS ARE MOVED IN SPEC PROBLEM]
...
void Problem::UpdateStates() {
    ...
    //[OVERRIDE FOLLOWING CODE IN THE SPEC PROBELM]
    // update prior
